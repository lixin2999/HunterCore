"""对象存储访问层：报告 sidecar / 评估文档统一存 MinIO（S3 协议，bucket 契约固定）。

生产实现基于 aioboto3（可选依赖，见服务 pyproject [minio] extra）；
单元测试以内存实现（app/tests/fakes.py）注入替换，遵循同一 Protocol。
"""
...
class ObjectStorage(Protocol):
    """对象存储访问协议（依赖倒置：仓储/服务仅依赖该接口）。"""

    async def get_json(self, key: str) -> dict[str, Any] | None:
        """读取 JSON 对象；key 不存在返回 None。"""
        ...
...
class S3ObjectStorage:
    ...
    def _create_client(self) -> Any:
        ...
        try:
            ...
            raise ServiceUnavailableError("对象存储客户端不可用（aioboto3 未安装）") from exc
    ...
    async def healthcheck(self) -> bool:
        ...
        except Exception:  # noqa: BLE001 - 健康探测失败即视为不可用（返回 False 降级），不向请求路径抛出
