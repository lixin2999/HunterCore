-- =====================================================================
-- HunterCore 单机部署包 — 时序库结构 timescaledb.sql
-- ⚠ GENERATED FILE（生成产物）：请勿手工编辑；单一事实来源 = contracts/database/ddl/05_timeseries.sql
-- 依赖：先执行 schema.sql（data_collector / data_analytics schema 与公共触发器函数已创建）
-- 关键约束（不可更改）：chunk_time_interval = 1 day；保留策略 90 天；主键必须包含分区键 time
-- 表清单（2 张 hypertable）
--   data_collector.vehicle_telemetry（车辆遥测，data-collector 批量写入）
--   data_analytics.algorithm_metrics（算法模块指标，module ∈ perception/planning/control）
-- 重新生成（仓库根目录执行）：
--   $ cat contracts/database/ddl/05_timeseries.sql > infra/deploy/sql/timescaledb.sql
-- 幂等：IF NOT EXISTS + create_hypertable(if_not_exists => TRUE) + add_retention_policy(if_not_exists => TRUE)
-- 执行：psql -U hunter -d hunter_core -f timescaledb.sql
-- =====================================================================
-- =====================================================================
