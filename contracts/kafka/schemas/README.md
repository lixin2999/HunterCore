...
| `event.schema.json` | `hunter.{vehicle_id}.event`、`event_raw` | `event_type` 19 种枚举、`event_level` 3 级枚举；类型↔等级映射由消费侧 `EVENT_LEVEL_BY_TYPE` 校验 |
...
| `remote_control.schema.json` | `hunter.{vehicle_id}.remote_control` | 20Hz；`seq` 单调递增；`target_velocity` ∈ [-2.0, 2.0] m/s；`heartbeat` 用于超时保护计时；G-10 可选扩展：`control.brake` 0-1（estop 恒 1.0）/`control.gear`（P/R/N/D 词表同 health）/`control.mode`（值域 pending #21）；顶层 `type`/`video`/`reason` 契约化（boot/stop 帧收对） |
| `analytics_result.schema.json` | `analytics_result` | `result_type` ∈ {metric, corner_case, report}；`trigger_event_type` 取受控事件词表（19 种）；`clip.pre_seconds/post_seconds` 固定 10（4.5 节事件前后各 10 秒）；含 `ego_trajectory`/`objects`/`environment` |
...
| `alert_event.schema.json` | `alert_event` | 6.2 节 Flink 实时告警（生产者 data-analytics）；`alert_type` 取 19 种受控事件类型、`level` 必须 = `EVENT_LEVEL_BY_TYPE[alert_type]`、`source_job` 取 5 个实时作业；`rule` 回带命中规则/比较符/阈值（阈值环境变量化）；幂等键 = `(vehicle_id, alert_id)`、延迟 ≤ 2s |
...
| `alert_event.alert_type` 取值域 | 复用受控事件类型 19 种（G-22② 新增 `collision_pre_warning`）；6.2.3 节 TTC<3.0s 的 warning 预警 → `collision_pre_warning`（与 collision_warning 分级），两档均投 `alert_event`（已结案，见 data-analytics 契约 pending #3） |
